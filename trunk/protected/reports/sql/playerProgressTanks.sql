...
FROM wot_player_tank_history wpth
  ...
  GROUP BY pth.player_id,
           pth.tank_id) last_updated_at ON last_updated_at.updated_at = wpth.updated_at AND last_updated_at.player_id = wpth.player_id AND last_updated_at.tank_id = wpth.tank_id
  ...
  AND pth.player_id = :player_id
  GROUP BY pth.player_id,
           pth.tank_id) pre_updated_at ON pre_updated_at.updated_at = wpth1.updated_at AND pre_updated_at.player_id = wpth1.player_id AND pre_updated_at.tank_id = wpth1.tank_id
